...
insert {
    graph <https://data.crow.nl/imbor/addendum/referentiemodellen> {
        ?uri
            a imbor-refmodels:RefAttribuut ;
            skos:prefLabel ?refAttribuutNL ;
    ...
    graph <csv:table/refModel_Attributen> {
        ?attr csv:refAttribuutGUID ?refAttribuutGUID ;
            csv:refInformatiemodel ?refInformatiemodel ;
            csv:refAttribuut ?refAttribuut ;

        optional { ?attr csv:Toelichting ?Toelichting . }
        optional { ?attr csv:URI ?URI . }
        optional { ?attr csv:Definitie ?Definitie . }

        filter (?refInformatiemodel != 15)  # QUDT wordt reeds anders verwerkt

        BIND ( STRLANG(?refAttribuut, "nl") as ?refAttribuutNL )
